...
class Encoder(nn.Module):
    ...
    def forward(self, input_ids: torch.LongTensor, attention_mask: torch.Tensor | None = None, position_ids: torch.LongTensor | None = None):
        """Args:
...
class Decoder(nn.Module):
    """Autoregressive transformer decoder that generates text conditioned on a latent vector."""
    def __init__(self, config):
        super().__init__()
        self.name = "DeepSeekASL-V1"
        self.config = config
        ...
        # Linear projection to condition on latent vector
        ...
        # Weight tying (only valid when token embeddings are used)
        if isinstance(self.embed_tokens, nn.Embedding):
            self.lm_head.weight = self.embed_tokens.weight

    def forward(self, latent: torch.Tensor, input_ids: torch.LongTensor, attention_mask: torch.Tensor | None = None, position_ids: torch.LongTensor | None = None):
        """Args:
            latent: [B, latent_dim] latent vector from encoder.
            input_ids: [B, T] decoder input token ids.
            attention_mask: optional [B, T] decoder mask.
            position_ids: optional [B, T] ids.
        Returns:
            logits: [B, T, vocab_size] token logits.
        """
        # Token embeddings
        hidden_states = self.embed_tokens(input_ids)  # [B, T, d]

        # Add latent conditioning
        latent_hidden = self.latent_to_hidden(latent).unsqueeze(1)  # [B, 1, d]
        hidden_states = hidden_states + latent_hidden  # broadcast along seq length
...
if __name__ == "__main__":
    """Run a quick smoke test of the VAE components."""
    import torch
    from summary import ModelSummary

    # Create lightweight configs for a fast test
    ...
    ModelSummary(encoder)
    decoder = Decoder(dec_cfg)
    ModelSummary(decoder)

    batch_size, seq_len = 2, 10
    dummy_input_ids = torch.randint(0, enc_cfg.vocab_size, (batch_size, seq_len))

    with torch.no_grad():
        latent = encoder(dummy_input_ids)
        print("Latent shape:", latent.shape)  # Expect [2, 32]

        decoder_input_ids = torch.randint(0, dec_cfg.vocab_size, (batch_size, seq_len))
        logits = decoder(latent, decoder_input_ids)
        print("Logits shape:", logits.shape)  # Expect [2, 10, 500]
